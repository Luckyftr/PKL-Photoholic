...
<style>
  ...
  /* SIDEBAR PELANGGAN (Sesuai Profil) */
  ...
  .userCard__avatar img{ width:100%; height:100%; object-fit:cover; }
  ...
  .menuItem{ width:100%; display:flex; align-items:center; gap:10px; min-height:44px; padding:10px 14px; border-radius:14px; text-decoration:none; color:var(--accent-red); font-weight:800; font-size:14px; border:1.5px solid transparent; background:transparent; cursor:pointer; transition:.18s ease; text-align:left; font-family:inherit; }
...
<div class="profileLayout">
  ...
  {{-- SIDEBAR PELANGGAN (Disamakan dengan Halaman Profil) --}}
  <aside class="sidebarCard">
    <div class="userCard">
      <div class="userCard__avatar">
        {{-- Gunakan auth()->user() agar aman meskipun dipanggil dari AuthController --}}
        <img src="{{ auth()->user()->photo ? asset('storage/'.auth()->user()->photo) : asset('img/pelanggan/image1.png') }}">
      </div>
        ...
        <a class="userCard__edit" href="{{ route('pelanggan.profile.index') }}">
          <span class="icon-inline">
            <svg viewBox="0 0 24 24">
              <path d="M12 20h9" stroke="currentColor" fill="none" stroke-width="2"/>
              <path d="M16.5 3.5l4 4L8 20H4v-4L16.5 3.5Z" stroke="currentColor" fill="none" stroke-width="2"/>
            </svg>
          </span>
          Ubah Profil
        </a>
    ...
    <div class="menuBlock">
      <div class="menuBlock__title">
        <svg viewBox="0 0 24 24">
          <path d="M12 12a4 4 0 1 0-4-4 4 4 0 0 0 4 4Z" stroke="currentColor" fill="none" stroke-width="2"/>
          <path d="M4.5 20c1.8-4 13.2-4 15 0" stroke="currentColor" fill="none" stroke-width="2"/>
        </svg>
        Akun Saya
      </div>
      ...
      <div class="menuList">
        ...
        {{-- Class is-active dipindahkan ke Ubah Kata Sandi --}}
        <a class="menuItem is-active" href="{{ route('pelanggan.password.form') }}">
          Ubah Kata Sandi
        </a>
        ...
        <button class="menuItem menuItem--danger" id="logoutBtn" type="button">
          Keluar
        </button>
      </div>
    </div>
    ...
    <div class="sidebarDecor">
      <img src="{{ asset('img/pelanggan/logo-icon.png') }}" alt="Dekorasi logo">
    </div>
  </aside>
    ...
    <form class="passForm" id="passForm">
      <div class="field">
        ...
        <div class="inputWrap">
          ...
          <button class="eyeBtn" type="button" data-target="oldPass" data-state="hidden" aria-label="Lihat password">
            <svg class="icon-eye icon-eye-open" viewBox="0 0 24 24"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z" fill="none" stroke="currentColor" stroke-width="2"/><path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" fill="none" stroke="currentColor" stroke-width="2"/></svg>
            <svg class="icon-eye icon-eye-off" viewBox="0 0 24 24"><path d="M3 3l18 18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M10.6 10.6a2.5 2.5 0 0 0 3.4 3.4" fill="none" stroke="currentColor" stroke-width="2"/><path d="M6.5 6.5C4 8.5 2 12 2 12s3.5 7 10 7c2 0 3.8-.5 5.3-1.4" fill="none" stroke="currentColor" stroke-width="2"/><path d="M9.5 4.3A10.8 10.8 0 0 1 12 5c6.5 0 10 7 10 7a18 18 0 0 1-3.2 4.5" fill="none" stroke="currentColor" stroke-width="2"/></svg>
          </button>
        </div>
      </div>
      ...
      <div class="field">
        ...
        <div class="strengthBox">
          <div class="strengthBar"><span id="strengthFill"></span></div>
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    ...
    passForm.addEventListener("submit", async (e) => {
      ...
      try {
        const response = await fetch("{{ route('pelanggan.password.update') }}", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Accept": "application/json", // TAMBAHAN PENTING UNTUK LARAVEL AJAX
        ...
        // Tangkap HTTP status code yang error (seperti 419, 500, 422)
        if (!response.ok) {
            // Jika error 422 (Validasi gagal dari Laravel)
            if (response.status === 422) {
                const errorData = await response.json();
                let errorMessages = "Data tidak valid.";
                if (errorData.errors) {
                     // Ambil pesan error pertama dari objek errors
                    errorMessages = Object.values(errorData.errors)[0][0]; 
        ...
        if (data.success) {
            openModal("Perubahan Berhasil Disimpan", data.message, [
             ...
             // Jika response JSON mengatakan success: false
            openModal("Gagal Menyimpan", data.message || "Gagal mengubah password.", [{ label: "Coba Lagi", className: "modalBtn--danger", onClick: closeModal }]);
        }

      } catch (error) {
          console.error("Fetch Error:", error); // Log error di console agar lebih mudah debugging
          
          // Tampilkan pesan error yang lebih spesifik jika ada
          let pesanError = "Gagal terhubung ke server. Silakan coba beberapa saat lagi.";
          if (error.message !== "Failed to fetch") {
             pesanError = error.message;
          }

          openModal("Terjadi Kesalahan", pesanError, [{ label: "Tutup", className: "modalBtn--cancel", onClick: closeModal }]);
      } finally {
